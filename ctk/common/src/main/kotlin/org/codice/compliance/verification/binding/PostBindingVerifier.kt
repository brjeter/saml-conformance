...
class PostBindingVerifier(override val reqData: SamlConfReqData,
                          override val response: Response) : BindingVerifier(reqData, response) {
    ...
    override fun decodeAndVerify(): org.w3c.dom.Node {
        ...
        val encodedSamlResponse = PostFormVerifier(reqData, response).verifyAndParse()
        val samlResponseDom = decode(encodedSamlResponse)
    ...
    override fun decodeAndVerifyError(): org.w3c.dom.Node {
        ...
        val encodedSamlResponse = PostFormVerifier(reqData, response).verifyAndParse()
        return decode(encodedSamlResponse)
    }
     ...
     */
    private fun decode(encodedSamlResponse: String): org.w3c.dom.Node {

        val decodedMessage: String
        try {
            decodedMessage = Decoder.decodePostMessage(encodedSamlResponse)
